...
├── README.md
└── oms/                               source documents, loaded by relative path
    ├── investment-thesis.pdf          the governing thesis
    ├── texas-portfolio-rgv.pdf        Sunstone 3-asset RGV portfolio
    ├── south-texas-mhc-portfolio.pdf
    ├── water-tower-rv-park.pdf
    ├── copper-ridge-rv-park.pdf
    ├── sugar-hill-rv-resort.pdf
    └── village-rv-park.pdf
```

`index.html` is self-contained (fonts, styles, scoring engine, and map are all inline). The only external references are the PDFs in `oms/`, which it loads by relative path.
...
The **Governing Thesis** section (and the Sources footnotes) also carry a **View Full Investment Thesis** button that opens `oms/investment-thesis.pdf` the same way.
...
The PDFs in `oms/` are **compressed copies** of the original broker Offering Memoranda — Marcus & Millichap for the five single-asset deals, Sunstone Real Estate Advisors for the RGV portfolio (rasterized at 105 DPI, ~2.5–5 MB each) so the repo stays light and the viewer loads fast. To use full-resolution originals, replace any file in `oms/` with the original PDF **using the same filename** — no code changes needed. The filename-to-deal mapping lives in the `OM_FILE` object near the top of the script in `index.html`.
